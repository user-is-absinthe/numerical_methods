...
def matplotlib_painter(size, old_points, new_points, path_to_old_file, crop):
    ...
    for i in range(len(new_points)):
        if len(crop) != 0:
            if crop[0] < new_points[i][0] < crop[1] and crop[2] < new_points[i][1] < crop[3]:
    ...
    if len(crop) == 0:
        ...
        plot([crop[0], crop[0]], [crop[2], crop[3]], 'r')  # left
        plot([crop[0], crop[1]], [crop[2], crop[2]], 'r')  # down
        plot([crop[0], crop[1]], [crop[3], crop[3]], 'r')  # up
        plot([crop[1], crop[1]], [crop[2], crop[3]], 'r')  # right

    for i in range(len(old_points)):
        scatter(old_points[i][0], old_points[i][1])

    # for i in range(len(old_points)):
    #     scatter(old_points[i][0], old_points[i][1])
    # xlabel(r'$x$')  # Метка по оси x в формате TeX
    # ylabel(r'$f(x)$')  # Метка по оси y в формате TeX
    # title(r'$aeeaef$')  # Заголовок в формате TeX
    grid(True)  # Сетка
...
def newton(path_to_file, arguments):
    ...
    if len(arguments) == 0:
        ...
        grid_array, size = read_img(path=path_to_file)
        underbreakble_old_points = copy(grid_array)
        grid_array = cut_the_grid(array=arguments, grid=grid_array)
...
def cut_the_grid(array, grid):
    ...
    for i in range(len(grid[0])):
        if grid[0][i][0] < array[0] or grid[0][i][0] > array[1]:
            to_del_x.append(i)
    for i in range(len(to_del_x)):
        del grid[0][len(to_del_x) - 1 - i]

    if len(array) == 4:
        to_del_y = []
        for i in range(len(grid[0])):
            if grid[0][i][1] < array[2] or grid[0][i][1] > array[3]:
                to_del_y.append(i)
        for i in range(len(to_del_y)):
            del grid[0][len(to_del_y) - 1 - i]
    '''
    return grid
