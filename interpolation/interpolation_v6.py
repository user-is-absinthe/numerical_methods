...
def matplotlib_painter(size, old_points, new_points, path_to_old_file, crop):
    ...
    if len(crop) != 0:
        # kx = (crop[1] - crop[0]) / (0 - size[1])
        # bx = crop[0]
        # ky = (crop[2] - crop[3]) / (0 - size[0])
        # by = crop[3]
        #
        ...
        #
        # plot([0, 0], [0, size[1] * ky + by], 'r')
        # plot([0, size[0] * kx + bx], [0, 0], 'r')
        # plot([0, size[0] * kx + bx], [size[1] * ky + by, size[1] * ky + by], 'r')
        # plot([size[0] * kx + bx, size[0] * kx + bx], [0, size[1] * ky + by], 'r')

        kx = (crop[0] - crop[1]) / (0 - size[1])
        bx = crop[0]
        ky = (crop[2] - crop[3]) / (0 - size[0])
        by = crop[3]
